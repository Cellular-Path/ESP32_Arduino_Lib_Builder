...
# Arduino branch to use when the vendored source is itself a git checkout.
if [ -z "$AR_BRANCH" ]; then
    AR_BRANCH="main"
fi
...
# Owner of the maintained builder repository.
AR_USER="Cellular-Path"

# IDF commit to use – pinned to the known-good Tasmota ESP-IDF 5.5 build
if [ -z "$IDF_COMMIT" ]; then
    export IDF_COMMIT="d47e9ffd41"
fi
...
AR_REPO="Cellular-Path/arduino-esp32"
IDF_REPO="tasmota/esp-idf"
...
if [ -n "$GITHUB_TOKEN" ]; then
    ...
    IDF_REPO_URL="https://$GITHUB_TOKEN@github.com/$IDF_REPO.git"
